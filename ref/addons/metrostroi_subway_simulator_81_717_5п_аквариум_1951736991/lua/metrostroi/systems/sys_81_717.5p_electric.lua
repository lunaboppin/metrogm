--------------------------------------------------------------------------------
-- 81-717.5P SPB electric schemes
--------------------------------------------------------------------------------
Metrostroi.DefineSystem("81_717.5P_Electric")
TRAIN_SYSTEM.MVM = 1
...
--]]
        ...
        Panel.UAVATriggered = BO*(1-Train.UAVAC.Value)
		--Panel.ABLow = BO*(Train.Battery.Voltage < 70)
		--Train:WriteTrainWire(110,BO*(Train.KRU))

        S["Radio"] = ((Train:ReadTrainWire(10) + (BO*Train.A56.Value)) * Train.AR63.Value) + (B*Train.AV3.Value)
        Train.ECRS.Power = S["Radio"]
        Panel.RVSLight = S["Radio"]
		Panel.KRUL = BO*KRU["14/1-B3"]
		Panel.KNL = BO
		Panel.PTK = BO
    else
        Panel.LSN = S["U2"]*T[18]
        ...
        Panel.TickerPower = B*Train.A53.Value
        Panel.AnnouncerPlaying = T[13]
    ...
    --B->A44->KMMK->23
    Train:WriteTrainWire(23,S["B3"]*(C(Train.VMKSwitch.Value == 0)))
    ...
    S["22B"] = T[10]*Train.A10.Value*C(Train.VMKSwitch.Value == 2)
    ...
    if isLVZ then
        if isKSD then
            ...
            Train:WriteTrainWire(7,S["TU"]+T[71]*Train.OhrSig.Value+AVO.Ring)
        end
        Panel.OhrSig = T[71]
    ...
    local BPSW_rez =  C(Train.BPSNSwitch.Value < 1)
    local BPSW_off =  C(Train.BPSNSwitch.Value <= 1)
    local BPSW_on =  C(Train.BPSNSwitch.Value > 1)
    S["36N"] = BO*Train.A45.Value
    Train:WriteTrainWire(37,Train.ConverterProtection.Value)
    Train:WriteTrainWire(69,S["36N"]*BPSW_off)
    Train:WriteTrainWire(36,T[69]*BPSW_on + S["36N"]*BPSW_rez)
    
    Panel.LKVP = T[36]
    Panel.LKVPR = T[36]*BPSW_rez
    Panel.RZP = T[36]*T[61]
    ...
    --FIXME ARS FMM2
    --F1->F2->BPF
    S["F8"] = S["F7"]*(Train.HeadLightsSwitch.Value)
    Train.Panel.Headlights1 = S["F8"]*Train.A46.Value
    Train.Panel.Headlights2 = S["F7"]*(Train.HeadLightsSwitch.Value-1)*Train.A47.Value
    if isKSD then
	...
	--Train:WriteTrainWire(122,Train.VMKSwitch.Value-1)
	--Train:WriteTrainWire(123,1-Train.VMKSwitch.Value)
	Panel.LMK = T[44]
	Panel.LMKR = T[23]
	Panel.VZ1L = T[48]
	Train:WriteTrainWire(108,Train.PneumaticNo2.Value)
	Panel.VZ2L = T[108]
...
local wires = {1,2,3,4,5,6,7,8,10,12,14,15,16,17,18,19,20,22,23,24,27,-28,28,25,13,-13,31,32,36,37,39,44,48,53,54,57,59,60,58,57,64,34,36,-34,61,64,66,67,68,69,-69,71,87,88,91,92,93,98,99,108,110}
