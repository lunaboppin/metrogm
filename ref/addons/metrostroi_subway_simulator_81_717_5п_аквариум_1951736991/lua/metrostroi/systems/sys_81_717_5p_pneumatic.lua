--------------------------------------------------------------------------------
-- 81-717.5P pneumatic system
--------------------------------------------------------------------------------
Metrostroi.DefineSystem("81_717_5P_Pneumatic")
...
function TRAIN_SYSTEM:Initialize(parameters)
    ...
    if not TURBOSTROI then
        ...
        local start = math.Rand(0.2,0.3)
        -- 0.6-1
        self.DoorSpeedMain = math.Rand(1.2,1.5)
...
function TRAIN_SYSTEM:Think(dT)
    ...
    if self.ValveType == 1 then
        ...
        -- 013: 1 Overcharge
        if (self.RealDriverValvePosition == 1) and BLDisconnect and (TLDisconnect or self.BrakeLinePressure > self.TrainLinePressure and 1.01*math.min(6.0,self.TrainToBrakeReducedPressure)) then
            self:equalizePressure(dT,"BrakeLinePressure", 1.01*math.min(6.0,self.TrainLinePressure), pr_speed)
        end
    ...
    if ((CurTime() - self.Timer > 0.10) and (self.DriverValvePosition > self.RealDriverValvePosition)) then
        self.Timer = CurTime()
		    if self.RealDriverValvePosition ~= 6 then
                Train:PlayOnce("br_013_5P","cabin")
			else
			    Train:PlayOnce("br_013_5P_6-7","cabin")
			end	
    ...
    if ((CurTime() - self.Timer > 0.10) and (self.DriverValvePosition < self.RealDriverValvePosition)) then
        self.Timer = CurTime()
		    if self.RealDriverValvePosition ~= 2 then
			    ...
			    Train:PlayOnce("br_013_5P_2-1","cabin")
			end	
        self.RealDriverValvePosition = self.RealDriverValvePosition - 1
    end
end
