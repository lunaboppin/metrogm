--------------------------------------------------------------------------------
-- АСОТП "ИГЛА" Белый ЦБКИ
--------------------------------------------------------------------------------
Metrostroi.DefineSystem("81_760_IGLA_CBKI1")
...
if CLIENT then
	...
	function TRAIN_SYSTEM:PrintText(x,y,text,col)
		...
		for i=1,#str do
			...
			local alpha = col.a
			if alpha > 1 then
				draw.SimpleText(char,"MetrostroiSubway_IGLAb",-6+(x+i)*25,22+y*36+1,ColorAlpha(col,alpha*0.08),TEXT_ALIGN_CENTER,TEXT_ALIGN_CENTER)
			end
			draw.SimpleText(char,"MetrostroiSubway_IGLA",-6+(x+i)*25,22+y*36,col,TEXT_ALIGN_CENTER,TEXT_ALIGN_CENTER)
		end
	end
  function TRAIN_SYSTEM:ClientThink()
	...
	function TRAIN_SYSTEM:IGLA()
		...
		if State == -3 then
			...
			self:PrintText(0,1,"ремонта асотп",colorf)
		elseif State == 0 then
	...
	function TRAIN_SYSTEM:Think(dT)
		local Train = self.Train
		if Train.Electric.Battery80V > 62 and Train.SF20.Value > 0.5 and self.State ~= -2  then
		...
		if Train.Electric.Battery80V < 62 or Train.SF20.Value < 0.5 or self.Reset then
			self.Reset = false
		...
		if self.State == -2 and Train.Electric.Battery80V > 62 and Train.SF20.Value > 0.5 then
			self.State = -1
