...
-- ������������� ����
--------------------------------------------------------------------------------
Metrostroi.DefineSystem("81_760_Electric")
...
function TRAIN_SYSTEM:Outputs()
	return {  "Brake", "Drive","V2","V1",
			 ...
			 "BTB","BTBr",
			 "MK","SD","EmerXod","UPIPower","Power","PowerReserve",
			"Recurperation","Itotal","Iexit","Chopper","ElectricEnergyUsed","ElectricEnergyDissipated","EnergyChange","BUFT"
		}
end
--[[
...
function TRAIN_SYSTEM:Think(dT,iter)
    ...
    --local power = self.Battery80V*self.SF6.Value > 62 
	--local powerReserve = self.Electric.Battery80V*(1-self.SF6.Value)*(self.RV.KRRPosition ~= 0 and 1 or 0) > 62 or power
    --local power1 = self.Electric.Battery80V > 62
    ...
    self.BUFT = P*Train.Battery.Value*Train.SF55.Value

	----------------------------------------------------------------------------
	-- Voltages from the third rail
	----------------------------------------------------------------------------
	-- ���������� � ������� ������������� �����
	--if Train.TR.Main750V < 550 and self.Main750V > 550 then self.Main750V = 500+Rand(0,40) end  
	...
	Train:WriteTrainWire(1,C(self.Battery80V > 67))-- and 1 or 0)
	...
	if RV then
		...
		Panel.LV = Train.Battery.Value*Train.SF44.Value*self.Battery80V
	end
	
	self.MK = Train.Battery.Value*PowerPSN*BUV.PSN*HV*Train.SF34.Value*(BUV.MK > 0 and 1 or Train:ReadTrainWire(10))

	Panel.WorkFan = P*Train.Battery.Value*Train.GV.Value*HV
	
	Panel.SalonLighting1 = P*Train.Battery.Value*Train.SF44.Value
	Panel.SalonLighting2 = P*Train.Battery.Value*Train.SF43.Value*BUV.MainLights
	...
	Async:TriggerInput("Power",BO*(Train.SF52 and Train.Battery.Value*Train.SF52.Value or 1)*Train.GV.Value*Train.BV.Value)--*HV)--*(1-BUKV.DisableTP))
    ...
    if Async.Mode<0 and Async.State>0 then
        self.Recurperation = C(self.Main750V > 749 and self.Main750V < 921)*BUV.Recurperation-- and 1 or 0
        ...
        self.Chopper = C(self.Main750V>=921 or self.Main750V<550)--  and 1 or 0
    else
        self.Recurperation = 0
        self.Iexit = 0
        self.Chopper = 0
