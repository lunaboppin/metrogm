...
--------------------------------------------------------------------------------
Metrostroi.DefineSystem("81_760_BUKP")
	...
	function TRAIN_SYSTEM:Think(dT)
		...
		if not VityazWork and self.State ~= (Power and -2 or 0) and (not Power or self.State ~= -3) then
			if self.State == 0 and self.State ~= -3 then
				self.State = -3
				self.State2 = Train.BKL and 1 or 0
			else
		...
		if Power then
			...
			Train:SetNW2Bool("VityazUOS",Train.BARS.UOSActive)											
			if self.State == 2 then
			...
			if self.State == 5 and (Train.SF6.Value == 1) then
				...
				if RV > 0 and not Back then
					...
					--if Train.SF6.Value > 0 then
						--self:CState("RVPB",--[[(Train.BARS.Speed < 1 and 1 or 0)*]] Train.Electric.V2*Train.SA1.Value*Train:ReadTrainWire(35) > 0)--(1-Train.RV["KRO5-6"])*Train.SF2.Value > 0)
					--end
					...
					elseif (countBL < 2 or Train.Pneumatic.RVTBTimer) and self.BLTimer then
						self.BLTimer = nil
					end
					for i=1,self.WagNum do
						local trainid = self.Trains[i]
					...
					--print(not Train.Prost_Kos.BlockDoorsR or Train.DoorBlock.Value == 1)					
					...
					self:CheckError(5.5,self.BLTimer and CurTime()-self.BLTimer > 0 and Train.RV.KRRPosition == 0 and Train:ReadTrainWire(33) == 0)	
					--print(max1-min1)
					...
					--Train:SetNW2Bool("PB",Train.PB.Value == 1)
					...
					Train:SetNW2Bool("VityazAB",BARS.AB)
					...
					if self.err == 0 then
						...
						elseif self.State2 == 51 then
							for i=1,self.WagNum do
								local train = self.Trains[self.Trains[i]]
								Train:SetNW2Int("VityazP"..i,train.BCPressure and train.BCPressure*10 or 0)
							end
			...
			--self:CState("RV",RV --[[*Train.SF2.Value*Train.SF3.Value]] > 0,"BUKP")
			...
			if Train:GetNW2Int("VityazMainMsg",0) < 2 then
				...
				if Train.SA15.Value > 0 and self.State == 5 then
					self.DoorTorec = true
				else
					self.DoorTorec = false
				end					
			end
	...
	function TRAIN_SYSTEM:PrintText(x,y,text,col,align,fix)
		...
		for i=1,#str do
			...
			if self.State ~= -3 then
				...
				if char == "." or char == ":" or char == "-" or char == "i" then x1=x1+1 end
	...
	function TRAIN_SYSTEM:VityazMonitor(Train)
		...
		elseif self.State == 5 then
			...
			self:PrintText(10,5,"БВ:",yellow,"right")
				for i=1,wagnum do
					if not Train:GetNW2Bool("VityazBattery"..i,false) or not Train:GetNW2Bool("VityazBUVState"..i,false) or not Train:GetNW2Bool("VityazSF"..i.."52",false) then
			...
			self:PrintText(10,6,"Сбор сх:",yellow,"right")
				for i=1,wagnum do
					if Train:GetNW2Bool("VityazBattery"..i,false) and Train:GetNW2Bool("VityazBUVState"..i,false) and Train:GetNW2Bool("VityazSF"..i.."52",false) then
			...
			self:PrintText(24,9,"РЕЖИМ: " .. (Train:GetNW2Bool("VityazUOS",false) and "УОС" or Train:GetNW2Bool("VityazAB",false) and "АБ" or self.RezhimARS),yellow)
			...
			elseif state2 == 11 then
				...
				self:PrintText(21,15,"МК",yellow)
					for i=1,wagnum do
						if Train:GetNW2Bool("VityazPVU"..i.."3",false) then
				...
				self:PrintText(21,22,"Торц.Дв",yellow)
					for i=1,wagnum do
						if Train:GetNW2Bool("VityazPVU"..i.."6",false) then
							self:PrintText(30+i,22,"Р",Train:GetNW2Bool("VityazDoorBlock"..i,false) and green or purple)
						else
							self:PrintText(30+i,22,"█",Train:GetNW2Bool("VityazDoorBlock"..i,false) and green or purple)
						end
					end
			...
			elseif state2 == 31 then
				...
				for i=1,wagnum do
					local DriveStrength = Format("%04.1f",Train:GetNW2Int("VityazDriveStrength"..i,0)/10)
					local BrakeStrength = Format("%04.1f",Train:GetNW2Int("VityazBrakeStrength"..i,0)/10)
					local EnginesStrength = Format("%06d",Train:GetNW2Int("VityazPower"..i,000000))
					self:PrintText(21,13+i,tostring(i),blue)
					self:PrintText(23,13+i,BrakeStrength,yellow)
					self:PrintText(28,13+i,DriveStrength,yellow)
					self:PrintText(34,13+i,EnginesStrength,yellow)
				end
			elseif state2 == 32 then
				...
				for i=1,wagnum do
					local I = (Train:GetNW2Int("VityazI"..i,0000) < 0 and "-" or "") .. Format("%04d",math.abs(Train:GetNW2Int("VityazI"..i,0000)))
					local U = (Train:GetNW2Int("VityazU"..i,0000) < 0 and "-" or "") .. Format("%04d",math.abs(Train:GetNW2Int("VityazU"..i,0000)))
					self:PrintText(21,13+i,tostring(i),blue)
			...
			elseif state2 == 41 then
				...
				for i=1,wagnum do
					...
					local IMK = Format("%02d",Train:GetNW2Int("VityazIMK"..i,0)/10)
					local UBS = Format("%02d",Train:GetNW2Int("VityazUBS"..i,0)/10)
					self:PrintText(21,13+i,tostring(i),blue)
