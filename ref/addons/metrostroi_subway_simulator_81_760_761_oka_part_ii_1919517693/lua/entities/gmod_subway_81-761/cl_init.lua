...
function ENT:UpdateTextures()
	...
	if self:GetNW2Bool("BKL",false) and self.Texture == "BKL" then
		...
		Metrostroi.Skins["train"][self.Texture].textures = {
			...
			["medium_vag"] = "models/metrostroi_train/81-760/medium_vag_blue_doors",
		}	
		for k,v in pairs(self:GetMaterials()) do
			if v == "models/metrostroi_train/81-760/hull" then
				self:SetSubMaterial(k-1,"models/metrostroi_train/81-760/hull_bkl")
			end
			if v == "models/metrostroi_train/81-760/medium_vag" then
				self:SetSubMaterial(k-1,"models/metrostroi_train/81-760/medium_vag_blue_doors")
			end	
		end	
	else
		Metrostroi.Skins["train"][self.Texture] = {}	
