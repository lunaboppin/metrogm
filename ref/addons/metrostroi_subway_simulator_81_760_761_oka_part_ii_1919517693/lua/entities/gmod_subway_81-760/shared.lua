...
ENT.Spawner = {
    ...
    {"Announcer","Spawner.760.Announcer","List",function()
        local Announcer = {}
        for k,v in pairs(Metrostroi.AnnouncementsASNP or {}) do if not v.riu then Announcer[k] = v.name or k end end
        return Announcer
    end},
    {"CISConfig","Spawner.760.CISConfig","List",function()
        local CISConfig = {}
        for k,v in pairs(Metrostroi.CISConfig or {}) do CISConfig[k] = v.name or k end
        return CISConfig
    end},	
    {"Scheme","Spawner.760.Schemes","List",function()
        local Schemes = {}
        for k,v in pairs(Metrostroi.Skins["760_schemes"] or {}) do Schemes[k] = v.name or k end
        return Schemes
    end},
    {"Version","Spawner.760.Version","List",{"Spawner.760.Version.STL","Spawner.760.Version.BKL"}, nil,function(ent,val,rot,i,wagnum,rclk)
		if not val then val = 1 end
		ent.STL = (val == 1)
		ent.BKL = (val == 2)
		ent:SetNW2Bool("STL",ent.STL)
		ent:SetNW2Bool("BKL",ent.BKL)
		ent:SetNW2String("Texture",(val == 2) and "BKL" or "STL")
		ent:SetNW2String("texture",(val == 2) and "BKL" or "STL")
    end},		
