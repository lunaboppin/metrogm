...
--timer.Simple(0,function()
	...
	if CLIENT then
		...
		TISU.Think = function(wag,...)
			...
			if ents then
				local rn = Format("%03d",wag:GetNW2String("RouteNumber","000"))
	...
	--смена положения трафарета конечной станции
	TISU.ClientProps.destination.pos = Vector(-0.6,0,0.6)
	...
	do return end
	--КВ
	local kv_type_index = -1
	local kv_type_read_table_name = "Тип КВ"
	...
	if not foundtable then
		table.insert(TISU.Spawner,#TISU.Spawner-1,{kv_type_table_name,kv_type_read_table_name,"List",{"Default","Old"}})
		kv_type_index = 2
	else
		kv_type_index = table.insert(TISU.Spawner[foundtable][4],"Old")
	end
	
	
	
	TISU.ClientProps["models/devil/kharkov/kv_718_base.mdl"] = {
		model = "models/devil/kharkov/kv_718_base.mdl",
		pos = Vector(),
		ang = Angle(0,0,0),
		hideseat=0.7
	}
	...
	UpdateCpropCallBack(
		...
		"controller",
		function(wag)
			if wag:GetNW2Int("KVTypeCustom",0) == kv_type_index then return "models/devil/kharkov/kh_kv_type2.mdl" end
		end,
		nil,
		function(wag,cent)
			if wag:GetNW2Int("KVTypeCustom",0) == kv_type_index then
				cent:SetPos(wag:LocalToWorld(Vector(449.1,16,-12.4)))
				cent:SetAngles(wag:LocalToWorldAngles(Angle(101,0,0)))
			end
		end
	)
	
	local oldstartedcallbackKV = TISU.UpdateWagNumCallBack
	TISU.UpdateWagNumCallBack = function(wag)
		wag:ShowHide("models/devil/kharkov/kv_718_base.mdl",true)
		oldstartedcallbackKV(wag)
	end
	
	UpdateCpropCallBack(
		TISU,
		"models/devil/kharkov/kv_718_base.mdl",
		nil,
		function(wag)
			if wag:GetNW2Int("KVTypeCustom",0) ~= kv_type_index then wag:ShowHide("models/devil/kharkov/kv_718_base.mdl",false)end
		end
	)
	
	--взлом метрострой екст
	timer.Simple(2,function()
	timer.Simple(0,function()
		for k,v in pairs(TISU.Spawner)do
			if istable(v) and v[1] == "OldKV" then
				v[2] = "ERROR"
				local oldthinkKV = TISU.Think
				TISU.Think = function(wag,...)
					wag:SetNW2Bool("OldKV",true)
					return oldthinkKV(wag,...)
				end
				break
			end
		end
