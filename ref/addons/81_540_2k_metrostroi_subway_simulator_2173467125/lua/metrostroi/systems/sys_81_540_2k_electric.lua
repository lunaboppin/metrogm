...
--------------------------------------------------------------------------------
Metrostroi.DefineSystem("81_540_2k_Electric")
TRAIN_SYSTEM.MVM = 1
...
function TRAIN_SYSTEM:SolveAllInternalCircuits(Train,dT,firstIter)
    ...
    local BO    = B*Train.VB.Value*Train.Pred2B.Value
    local HightVoltage = Train.Electric.Aux750V 
    ...
    S["B3"] = B*Train.A44.Value*Train.Pred1B.Value
    S["14a"] = S["B3"]*KRU["14/1-B3"]
    S["14b"] = S["14a"]*Train.A17.Value
    S["F"] = T[10]*Train.A29.Value
    S["F7"] = S["F"]*KV["F-F7"]+S["14b"]*KRU["11/3-FR1"]  
    Train:WriteTrainWire(5,S["10AK"]*KV["10AK-5"] + KRU["5/3-ZM31"]*(Train.KD.Value+Train.VAD.Value)*-10)
    ...
    ARS.PD = T[88]
    local als_och =  C(Train.ALSToggleSwitch.Value == 0)
    local als_off =  C(Train.ALSToggleSwitch.Value == 1)
    local als_on =  C(Train.ALSToggleSwitch.Value == 2) 
    ARS.ALS = T[10]*Train.A43.Value*als_on+als_off*Train.ARS.Value+als_off*T[87]
    Train.BLPM.Power = ARS.ALS
    ARS.NGPower = S["7D"]*RC1
    Train.BIS200.Power = T[10]*Train.A43.Value*(als_on+Train.ARS.Value)*Train.AIS.Value
    ARS.KB=T[91]*RC1+(ARS.ALS*(1-Train.BSM_GE.Value)+ARS.GE*Train.BSM_GE.Value)*Train.KVT.Value
    Train.BSM_KRT:TriggerInput("Set",(max(0,T[6])+max(0,T[8]))*RC1*Train.A40.Value)
    ...
    Train:WriteTrainWire(92,ARS.DA*RC1*Train.BUM_RVD1.Value+S["7D"]*(1-RC1)*Train.A59.Value*Train.KAH.Value)
    Train:WriteTrainWire(93,ARS.GE*Train.BUM_RVD2.Value*RC1+S["B3"]*(1-RC1)*Train.A58.Value*Train.KAH.Value)
    ...
    Train.R1_5:TriggerInput("Set",S["19B"]*Train.RV2.Value*(Train.AVU.Value+Train.OtklAVU.Value))
    ...
    --
    --Вспом цепи
    Train:WriteTrainWire(10,BO*Train.A56.Value*Train.Pred2B.Value) 
    --B->A44->KMMK->23
    ...
    Train:WriteTrainWire(69,S["36N"]*Train.BPSNon.Value+Train.RezBPSNon.Value)
    Train:WriteTrainWire(36,T[69]*(1-Train.BPSNon.Value+Train.RezBPSNon.Value)) 
    Train:WriteTrainWire(101,Train.VB.Value*S["F7"])
    Train:WriteTrainWire(102,Train.A83.Value*Train.UnBlockDoor.Value*S["F7"]) 
    Train:WriteTrainWire(100,T[36]*BO*(1-Train.NR.Value)) 
    Panel.LKVP = T[36]
    Panel.LZBP = T[100]+T[36]*T[61] 
    Panel.RZP = T[36]*T[61]
    ...
    Train:WriteTrainWire(16,S["D1"]*(Train.VUD1.Value*Train.VUD2.Value+(Train.ALSCoil.Speed > 5 and 1 or 0))) 
    Panel.DR1 = T[101]*C(Train.ALSCoil.Speed<5)*(1-Train.DoorSelect.Value) 
    Panel.DR2 = T[101]*C(Train.ALSCoil.Speed<5)*Train.DoorSelect.Value
...
function TRAIN_SYSTEM:SolveRKInternalCircuits(Train,dT,firstIter)
    ...
    local RheostatController = Train.RheostatController
    local RK    = RheostatController.SelectedPosition
    local B     = (Train.Battery.Voltage > 55) and 1 or 0*Train.Pred2B.Value
    local BO    = B*Train.VB.Value*Train.Pred2B.Value
    local T     = Train.SolverTemporaryVariables
...
local wires = {1,2,3,4,5,6,7,8,10,12,14,15,16,17,18,19,20,22,23,24,27,-28,28,25,13,-13,31,32,36,37,39,44,48,53,54,57,59,60,58,57,64,34,36,-34,61,64,66,67,68,69,71,87,88,91,92,93,98,99,100,101,102,103}
