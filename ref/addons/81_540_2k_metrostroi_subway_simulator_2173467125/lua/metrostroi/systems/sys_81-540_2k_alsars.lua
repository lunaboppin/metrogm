--------------------------------------------------------------------------------
-- Система АРС-АЛС для состава 81-540.2K  (БАРС не барс конечно)
--------------------------------------------------------------------------------
-- Copyright (C) 2013-2018 Metrostroi Team & FoxWorks Aerospace s.r.o.
-- Contains proprietary code. See license.txt for additional information.
--------------------------------------------------------------------------------
Metrostroi.DefineSystem("81_540_2k_alsars")
function TRAIN_SYSTEM:Initialize()
    ...
    self.Train:LoadSystem("BIS200","81_540_2k_BIS200")
...
function TRAIN_SYSTEM:Think(dT)
   ...
   -- print(ALS.Speed)
    ...
    Train.NG:TriggerInput("Set",S["FMM_28"]*Train.FMM1.Value+self.NGPower*Train.NG.Value*(1-Train.NH.Value)+Train.VN.Value)
    ...
    local EnableALS = ((S["GE"]+self.ALS)--[[ *Train.KPK1.Value*Train.ALS.Value+(1-Train.KPK2.Value)--]] )*Train.FMM1.Value
	...
	if Train.KD.Value ~= self.KDValue then
		if Train.KD.Value == 1 then
			self.PNOffTimer = CurTime()+3.5
		end
		self.KDValue = Train.KD.Value 
    end
    if Train.KD.Value == 0 and ALS.Speed < 1 then 
       
         self.PNOffTimer = 0
    end 

    self["48"] = self.GE*(Train.BUM_RET.Value+Train.BUM_RVZ1.Value)*(CurTime()-self.PNOffTimer > 0 and 1 or 0)
    ...
    --LN 1/5 Fix
    Train:WriteTrainWire(89,S["GE"]*(Train.NG.Value*Train.FMM1.Value+Train.NH.Value*(1-Train.FMM1.Value))*Train.PD2.Value+Train.VN.Value)
    ...
    --LN 1/5 Fix
    Panel.LN = S["GE"]*Train.NG.Value*Train.PD1.Value+Train:ReadTrainWire(89)*Train.FMM1.Value+Train.VN.Value
     --if ALS.Speed < 5   then 
