...
ENT.SyncTable = {
    ...
    "CustomE","CustomF","CustomG","R_UNch","R_ZS","R_G","R_Radio","R_Program1","R_Program2","R_Program1H","RezBPSNon","KVT","KVTR",
    "VZ1","VUD1","KDL","KDLR","KDLK","KDP","KDLRK","DoorSelect","Last_Next","Last_Prev",
    "KRZD","R_VPR","VozvratRP","AVU","KVP","ConverterProtection","RZP","UnBlockDoor","UNBDK",
    "KSN","ARS","ARSR","ALS","OtklAVU","OtklBV","OtklBVK","OVT","L_1","L_2","L_3","VP","DIPoff", 
    "VMK","BPSNon","RezMK","ARS13","L_4","VUS","VAH","VAD","KRP",
    "KAH","KAHK","KDPK","Pred1","Pred2","Pred1B","Pred2B","OHSteklo","OHMirror",
    ...
    "AV2","AV3","AV4","AV5","AV6","AV1","A79","A83","A82",
    "AIS","A15","A81","A68","A80","INF_TabloNext","INF_TabloPrev",
    "RC1","VB","BPS","UOS", "PB", "UAVA","ALSToggleSwitch","VVToggleSwitch","VV1ToggleSwitch","SAB1ToggleSwitch", 
    "DriverValveBLDisconnect","DriverValveTLDisconnect","DriverValveDisconnect","ParkingBrake","EPK","EmergencyBrakeValve",
    "VUD2","VDL","Wiper", "GV",
     "ALSFreq","Ring","R_ASNPOn",
    "V11","V12","V13","SAB1","VN","HeadO","MirrorO",
    "INF_1","INF_2","INF_3","INF_4","INF_5","INF_6","INF_7","INF_8","INF_9","INF_Prev","INF_Next","INF_Play","INF_Stop",
}
ENT.SyncFunctions = {
    ""
}
function ENT:Initialize()
    ...
    self.MaskType = 1
    self.LampType = 1
    self:SetModel("models/metrostroi_train/540kyiv/540kyiv_body.mdl")
    ...
    self.Lights = {
        ...
        -- Reverse
        [8] = { "light",Vector(480,-45, 52), Angle(0,0,0), Color(255,50,50),     brightness = 0.2, scale = 2.5, texture = "sprites/light_glow02.vmt"  },
        [9] = { "light",Vector(480, 45, 52), Angle(0,0,0), Color(255,50,50),     brightness = 0.2, scale = 2.5, texture = "sprites/light_glow02.vmt"  },

        -- Cabin
        [10] = { "dynamiclight",        Vector( 425, 0, 30), Angle(0,0,0), Color(216,161,92), distance = 550, brightness = 0.25},

        -- Interior
        [11] = { "dynamiclight",    Vector( 200, 0, 2), Angle(0,0,0), Color(255,245,245), brightness = 3, distance = 400 , fov=180,farz = 128 },
        [12] = { "dynamiclight",    Vector(   0, 0, 2), Angle(0,0,0), Color(255,245,245), brightness = 3, distance = 400, fov=180,farz = 128 },
        [13] = { "dynamiclight",    Vector(-200, 0, 2), Angle(0,0,0), Color(255,245,245), brightness = 3, distance = 400 , fov=180,farz = 128 },
...
function ENT:TrainSpawnerUpdate()
    self:SetNW2Int("BPSNType",13)
    self:SetNW2Int("RingType",3)
    local OTLSkins = { "Default5402kKiev", "Default5402kKievWhite" } 
    local SPLSkins =  { "Default5402kKievGray", "Default5402kKievSplYellow" } 
    self:SetNW2String("CabTexture",table.Random(self:GetNW2Int("TypeLine",1)==1 and OTLSkins or SPLSkins))
    self:SetNW2String("Texture","Default540kiev")
    self:SetNW2Int("BPSNType",13)
    self:SetNW2Bool("Bochok",math.random()>0.5)
    self:SetNW2Int("Crane",2)
    ...
    self:SetPackedBool("Crane013",true)
    self:SetNW2Float("Crane013Loud",1)
...
function ENT:Think()
    ...
    self:SetPackedBool("RedLights_l",Panel.RedLight2 > 0)
    self:SetPackedBool("RedLights_r",Panel.RedLight1 > 0)
    self:SetPackedBool("CabLights",Panel.CabLights>0)
    self:SetPackedBool("EqLights",Panel.EqLights>0)
    self.BlockDoor = self:ReadTrainWire(102)*(self.RearDoor and 0 or 1) 
    ...
    self:SetLightPower(8,Panel.RedLight1>0,1)
    self:SetLightPower(9,Panel.RedLight2>0,1)
    ...
    self:SetPackedBool("LZBP",Panel.LZBP > 0.5) 
    self:SetPackedBool("DR1", Panel.DR1 > 0)
    self:SetPackedBool("DR2", Panel.DR2 > 0)
    ...
    if CurTime()-self.SpeedoTimer > 0.4 then
        ...
        self.SpeedLag = math.max(0,self.Speed+(speed-self.Speed)*(0.4+math.max(0,math.min((self.Speed-5)*0.2,0.4))))
    ...
    self:SetPackedRatio("BatteryVoltage",self.Panel["V1"]*self.Battery.Voltage/150.0)--BatteryVoltageA
    self:SetPackedRatio("BatteryVoltageA",self.A51.Value*self:ReadTrainWire(36)*self.Panel["V1"]*self.Battery.Voltage/150.0)
    ...
    self:SetPackedBool("Pred1B",self.Pred1B.Value)
    self:SetPackedBool("Pred2B",self.Pred2B.Value)
    ...
    self:SetPackedRatio("SAB1ToggleSwitch",self.SAB1ToggleSwitch.Value/2)
    self:SetPackedRatio("VVToggleSwitch",self.VVToggleSwitch.Value/4)
    self:SetPackedRatio("VV1ToggleSwitch",self.VV1ToggleSwitch.Value/4)
    self:SetNW2Int("VV1ToggleSwitch2",self.VV1ToggleSwitch.Value)
    self:SetNW2Int("VVToggleSwitch2",self.VVToggleSwitch.Value)
    -- Update ARS system
...
function ENT:OnButtonPress(button,ply)
    ...
    if button == "RearDoor" and self.BlockDoor ~= 1 then self.RearDoor = not self.RearDoor end
    ...
    for i = 1, 2 do 
       if button == "Pred" .. i .. "BToggle" and self.KVC.Value > 0 then 
            ply:Kill() 
    ...
    if self:GetNW2Int("TypeLine") == 2 then 
        self.KV:TriggerInput("ControllerUnlock",1)
    else 
...
function ENT:OnButtonRelease(button)
   ...
   if self:GetNW2Int("TypeLine") == 2 then 
    ...
    self.KV:TriggerInput("ControllerUnlock",0) 
  end 
